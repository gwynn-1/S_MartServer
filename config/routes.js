/**
 * Route Mappings
 * (sails.config.routes)
 *
 * Your routes tell Sails what to do each time it receives a request.
 *
 * For more information on configuring custom routes, check out:
 * https://sailsjs.com/anatomy/config/routes-js
 */

module.exports.routes = {


  //  ╦ ╦╔═╗╔╗ ╔═╗╔═╗╔═╗╔═╗╔═╗
  //  ║║║║╣ ╠╩╗╠═╝╠═╣║ ╦║╣ ╚═╗
  //  ╚╩╝╚═╝╚═╝╩  ╩ ╩╚═╝╚═╝╚═╝

  /***************************************************************************
  *                                                                          *
  * Make the view located at `views/homepage.ejs` your home page.            *
  *                                                                          *
  * (Alternatively, remove this and add an `index.html` file in your         *
  * `assets` directory)                                                      *
  *                                                                          *
  ***************************************************************************/
 "GET /api/gateway/verify-otp/:token":"SignupController.verifyotp",
  '/': {
    view: 'pages/homepage'
  },

  /***************************************************************************
  *                                                                          *
  * More custom routes here...                                               *
  * (See https://sailsjs.com/config/routes for examples.)                    *
  *                                                                          *
  * If a request to a URL doesn't match any of the routes in this file, it   *
  * is matched against "shadow routes" (e.g. blueprint routes).  If it does  *
  * not match any of those, it is matched against static assets.             *
  *                                                                          *
  ***************************************************************************/


  //  ╔═╗╔═╗╦  ╔═╗╔╗╔╔╦╗╔═╗╔═╗╦╔╗╔╔╦╗╔═╗
  //  ╠═╣╠═╝║  ║╣ ║║║ ║║╠═╝║ ║║║║║ ║ ╚═╗
  //  ╩ ╩╩  ╩  ╚═╝╝╚╝═╩╝╩  ╚═╝╩╝╚╝ ╩ ╚═╝

  'POST /api/gateway/login':'LoginController.login',
  'POST /api/gateway/logout':"LoginController.logout",
  'POST /api/gateway/signup':"SignupController.signup",
  'POST /api/gateway/check-login':'LoginController.checkLogin',

  'GET /api/qrcode/get':'QrCodeController.getQrCode',

  'GET /api/user/get':'UserController.getUserInfo',
  'POST /api/user/update':'UserController.updateUser',
  'POST /api/user/update-avatar':'UserController.updateAvatar',

  'GET /api/product/get':'ProductController.getAllProduct',

  'GET /api/location/get-ward':'LocationController._getWard',
  'GET /api/location/get-district':'LocationController._getDistrict',
  'GET /api/location/get-province':'LocationController._getProvince',

  'GET /api/receipt/:id?':'ReceiptController._getReceipt',
  'GET /api/note/:id?':'NoteController._getNote',

  'GET /api/cart/list':'CartController._listCart',
  'POST /api/cart/update':'CartController._modifyCart',
  
  //use for embedded camera device
  'POST /api/qrcode/authen':'QrCodeController.authenQrCode'
  // 'POST /api/qrcode/authen':'QrCodeController.authenQrCode'

  //  ╦ ╦╔═╗╔╗ ╦ ╦╔═╗╔═╗╦╔═╔═╗
  //  ║║║║╣ ╠╩╗╠═╣║ ║║ ║╠╩╗╚═╗
  //  ╚╩╝╚═╝╚═╝╩ ╩╚═╝╚═╝╩ ╩╚═╝


  //  ╔╦╗╦╔═╗╔═╗
  //  ║║║║╚═╗║
  //  ╩ ╩╩╚═╝╚═╝


};
